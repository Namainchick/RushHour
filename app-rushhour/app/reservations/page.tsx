...
function StatusPill({ status }: { status: Reservation["status"] }) {
  const map = {
    angefragt: { label: "Angefragt", cls: "bg-amber-100 text-amber-700" },
    bestaetigt: { label: "Bestätigt", cls: "bg-emerald-100 text-emerald-700" },
  } as const;
  const s = map[status];
...
export default function ReservationsPage() {
  ...
  return (
    <main className="mx-auto max-w-4xl px-5 pt-10 pb-24">
      <h1 className="text-3xl font-bold text-ink">Deine Reservierungen</h1>
      <p className="mt-1 text-muted">Creator-Kollaborationen, die du angefragt hast.</p>

      {reservations.length === 0 ? (
        <div className="mt-12 flex flex-col items-center rounded-3xl bg-cloud px-6 py-16 text-center">
          <span className="text-5xl">🤝</span>
          <h2 className="mt-4 text-xl font-semibold text-ink">Noch keine Reservierungen</h2>
          <p className="mt-1 max-w-sm text-muted">
            Finde passende Creator und frag deine erste Kollaboration an — sie erscheint dann hier.
          </p>
          <Link
            href="/"
              ...
              <div className="flex flex-1 flex-col p-4 sm:py-5 sm:pr-5">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted">
                  ...
                  {typeof r.priceFrom === "number" && (
                    <span>
                      ab <span className="font-semibold text-ink">{r.priceFrom.toLocaleString("de-DE")} €</span>
                    </span>
                  )}
                  <span>Angefragt am {r.createdAtLabel}</span>
                </div>
